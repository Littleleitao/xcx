...
/**
 * 根据客户端传过来的数据和数据库做比较,来获取传过来产品的状态
 ...
 */
    ...
    {
        //$oPID在$products的序号默认为-1
        $pIndex = -1;
        //$pStatus保存每一个订单里面的详细信息
        $pStatus = [
            'id' => null,
            'haveStock' => false,
            'count' => 0,
    ...
    // 根据订单查找真实商品,$oProducts传过来的是一个二维数组
    private function getProductsByOrder($oProducts)
    {
        $oPIDs = [];
        //查找数据库库存不要直接循环数据库,否则会把数据库服务器搞垮
        //把传过来的$oProducts的Product_id全部放在一个数组里面
        foreach ($oProducts as $item) {
            array_push($oPIDs, $item['product_id']);
        }
        // 为了避免循环查询数据库,通过传过来的商品id来查询数据库的商品id
